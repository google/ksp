 ...
 * application. The application can either be an android app or jvm app. Test must call
 * [setupAppAsAndroidApp] or [setupAppAsJvmApp] before using the [runner].
 */
class KspIntegrationTestRule(
    ...
    fun runner(): GradleRunner {
        ...
        return GradleRunner.create().withProjectDir(testProject.rootDir)
    }
     ...
     *
     * The passed argument must be a class with a name (e.g. not inline) as it will be added to the
     * classpath of the processor and will be re-loaded when the test runs. For this reason, these
     * classes cannot access to the rest of the test instance.
     */
    fun addProvider(provider: KClass<out SymbolProcessorProvider>) {
        val qName =
            checkNotNull(provider.java.name) {
     ...
     * file and necessary gradle configuration. If [enableAgpBuiltInKotlinSupport] is true, enable
     * AGP's built-in Kotlin support instead of applying the Kotlin Android Gradle plugin. If
     * [applyKspPluginFirst] is true, apply the KSP plugin first.
     */
    fun setupAppAsAndroidApp(
        ...
        if (!enableAgpBuiltInKotlinSupport) {
            ...
            testProject.appModule.plugins.add(
    ...
    /**
     * Sets up the app module as a multiplatform app with the specified [targets], wrapped in a
     * kotlin { } block.
     */
    ...
    private fun addAndroidBoilerplate() {
        ...
        testProject.appModule.moduleRoot
