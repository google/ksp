 ...
 */
open class KspMultiplatformExtension {
    internal val kspExtension = KspExtension() // global options
    ...
    // - A corresponding property in `kspExtension` provides the global variant's backing field.

    /** Options passed to the processor (global). */
    open val Project.arguments: Map<String, String> get() = kspExtension.arguments
    ...
    open fun Project.arg(k: String, v: String) = kspExtension.arg(k, v)
    ...
    open fun arg(arg: CommandLineArgumentProvider) = kspExtension.arg(arg)

    /** Block other compiler plugins by removing them from the classpath (global option). */
    open var blockOtherCompilerPlugins: Boolean by kspExtension::blockOtherCompilerPlugins
     ...
     * Use your discretion.
     */
    open var allowSourcesFromOtherPlugins: Boolean by kspExtension::allowSourcesFromOtherPlugins

    /** Treat all warnings as errors (global option). */
    open var Project.allWarningsAsErrors: Boolean by kspExtension::allWarningsAsErrors

    /** Treat all warnings as errors (source set-specific option). */
    open var KotlinSourceSet.allWarningsAsErrors: Boolean
        get() = sourceSetOptions(this).allWarningsAsErrors ?: kspExtension.allWarningsAsErrors
    ...
    internal fun globalSourceSetOptions(): SourceSetOptions = kspExtension.globalSourceSetOptions()
}

internal fun KspExtension.globalSourceSetOptions(): SourceSetOptions = SourceSetOptions().also {
    it.inheritable = true
    it.enabled = false
 ...
 *
 * If [inheritable] is true (the default),
 * – source set options with a null value inherit their values from parent source sets in the source set hierarchy,
 * – [apOptions] / [arguments] inherit all key/value pairs for keys that are not already present.
 * Inheritance can be disabled for a source set, but[inheritable] can be
 */
