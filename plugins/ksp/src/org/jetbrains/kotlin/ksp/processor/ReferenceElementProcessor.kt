...
open class ReferenceElementProcessor: AbstractTestProcessor() {
    ...
    override fun process(resolver: Resolver) {
        ...
        val sortedReferences = references.filter { it.element is KSClassifierReference }.sortedBy { (it.element as KSClassifierReference).referencedName() }
        ...
        for (i in sortedReferences)
            results.add("Qualifier of ${i.element} is ${(i.element as KSClassifierReference).qualifier}")
