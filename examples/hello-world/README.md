...
- `annotations`: This subproject defines the `HelloWorld` annotation. By keeping annotations in a separate
...
- `processor`: This subproject defines a minimal processor that selects functions annotated with
  `HelloWorld` and generates a function that prints a message.
- `app`: This subproject defines a single `main` that calls the generated function.
  The `main` function is annotated with `HelloWorld` to trigger the processor.
...
- `resolver.getSymbolsWithAnnotation(...)`: To find all symbols annotated with our fully qualified
  annotation name (`com.example.annotations.HelloWorld`).
...
- `implementation(project(":annotations"))`: Gives the app access to the `HelloWorld` annotation
  definition.
