...
class DefaultFunctionProcessor : AbstractTestProcessor() {
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        val ktInterface =
            resolver.getClassDeclarationByName(resolver.getKSNameFromString("KTInterface"))
                as KSClassDeclaration
        val javaInterface =
            resolver.getClassDeclarationByName(resolver.getKSNameFromString("C"))
                as KSClassDeclaration
        ...
        val containsFun =
            ktInterface.getAllFunctions().single {
        ...
        val interfaceProperty =
            ktInterface.declarations.single {
        ...
        val nonAbstractInterfaceProp =
            ktInterface.declarations.single {
                it.simpleName.asString() == "nonAbstractInterfaceProp"
            } as KSPropertyDeclaration
        result.add(
        ...
        val abstractClass =
            resolver.getClassDeclarationByName(resolver.getKSNameFromString("B"))
                as KSClassDeclaration
        result.add("${abstractClass.simpleName.asString()}: ${abstractClass.isAbstract()}")
        val parameterVal =
            abstractClass.declarations.single {
        ...
        val aProperty =
            abstractClass.declarations.single { it.simpleName.asString() == "a" }
                as KSPropertyDeclaration
        ...
        D.getAllProperties().forEach {
            result.add("${it.simpleName.asString()}: isMutable: ${it.isMutable}")
        }
