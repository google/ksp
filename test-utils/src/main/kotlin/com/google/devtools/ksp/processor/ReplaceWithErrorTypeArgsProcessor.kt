...
open class ReplaceWithErrorTypeArgsProcessor : AbstractTestProcessor() {
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        for (decl in decls) {
            ...
            results.add(
                "$declName.star.replace($xargs): ${decl.asStarProjectedType().replace(xargs)}"
            )
            results.add(
                "$declName.star.replace($yargs): ${decl.asStarProjectedType().replace(yargs)}"
            )
        ...
        val function =
            resolver.getFunctionDeclarationsByName(resolver.getKSNameFromString("f"), true).single()
        results.add(
            "default type:${function.parameters.single().type.resolve().replace(emptyList())}"
        )
        // TODO: fix flexible type creation once upstream available.
        val js1 = resolver.getClassDeclarationByName("JS1")!!
        results.add(
            "flexible type star:${js1.getDeclaredProperties().single().type.resolve().starProjection()}"
        )
        ...
        resolver.getClassDeclarationByName("ErrorSuper")?.let {
            results.add(
                "replace error type get: ${it.superTypes.single().resolve().replace(emptyList())}"
            )
        }
        return emptyList()
    }
