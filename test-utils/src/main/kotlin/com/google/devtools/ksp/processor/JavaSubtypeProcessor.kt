...
class JavaSubtypeProcessor : AbstractTestProcessor() {
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        val javaCollection =
            resolver.getJavaClassByName("kotlin.collections.Collection")!!.asStarProjectedType()
        val javaSet = resolver.getJavaClassByName("kotlin.collections.Set")!!.asStarProjectedType()
        val ktFunctionJava =
            resolver.getJavaClassByName("kotlin.jvm.functions.Function0")!!.asStarProjectedType()
        val ktCollection =
            resolver
                .getClassDeclarationByName("kotlin.collections.Collection")!!
                .asStarProjectedType()
        val ktSet =
            resolver.getClassDeclarationByName("kotlin.collections.Set")!!.asStarProjectedType()
        val ktFunction =
            resolver
                .getClassDeclarationByName("kotlin.jvm.functions.Function0")!!
                .asStarProjectedType()
        ...
        val javaSetOfJavaSet =
            javaSet.replace(listOf(resolver.getTypeArgument(javaSetRef, Variance.INVARIANT)))
        val javaSetOfKtSet =
            javaSet.replace(listOf(resolver.getTypeArgument(ktSetRef, Variance.INVARIANT)))
        val ktSetOfJavaSet =
            ktSet.replace(listOf(resolver.getTypeArgument(javaSetRef, Variance.INVARIANT)))
        val ktSetOfKtSet =
            ktSet.replace(listOf(resolver.getTypeArgument(ktSetRef, Variance.INVARIANT)))
        val javaCollectionOfJavaSet =
            javaCollection.replace(listOf(resolver.getTypeArgument(javaSetRef, Variance.INVARIANT)))
        val javaCollectionOfKtSet =
            javaCollection.replace(listOf(resolver.getTypeArgument(ktSetRef, Variance.INVARIANT)))
        val ktCollectionOfJavaSet =
            ktCollection.replace(listOf(resolver.getTypeArgument(javaSetRef, Variance.INVARIANT)))
        val ktCollectionOfKtSet =
            ktCollection.replace(listOf(resolver.getTypeArgument(ktSetRef, Variance.INVARIANT)))
        val ktCollectionOfJavaCollection =
            ktCollection.replace(
                listOf(resolver.getTypeArgument(javaCollectionRef, Variance.INVARIANT))
            )
        val javaSetOfKtCollection =
            javaSet.replace(listOf(resolver.getTypeArgument(ktCollectionRef, Variance.INVARIANT)))
        val container = resolver.getClassDeclarationByName("Container")!!
        val strRef =
            resolver.getTypeArgument(
                (container.declarations.single { it.simpleName.asString() == "str" }
        ...
        val kotlinFunctionImplementorJava =
            resolver.getClassDeclarationByName("IntSupplier")!!.asStarProjectedType()
        ...
        isOk =
            isOk &&
                ktFunctionJava.isAssignableFrom(ktFunction) &&
                ktFunction.isAssignableFrom(ktFunctionJava)
