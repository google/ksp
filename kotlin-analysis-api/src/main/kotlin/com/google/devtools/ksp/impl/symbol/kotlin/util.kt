...
internal fun KaValueParameterSymbol.getDefaultValue(): KaAnnotationValue? = this.psi.let { psi ->
    // Define predicates for readability
    val isJvmCompilation = ResolverAAImpl.instance.isJvm
    val isKLibCompilation = !isJvmCompilation
    when {
        psi is KtParameter -> analyze {
            psi.defaultValue?.evaluateAsAnnotationValue()
        }

        // ClsMethodImpl means the psi is decompiled psi.
        psi is ClsMemberImpl<*> && this is KaFirValueParameterSymbol && isJvmCompilation ->
            getDefaultValueOnJvm(ResolverAAImpl.instance.javaFileManager, this)

        psi == null && this is KaFirValueParameterSymbol && isJvmCompilation ->
            getDefaultValueOnJvm(ResolverAAImpl.instance.javaFileManager, this)
        ...
        this is KaFirValueParameterSymbol && isKLibCompilation ->
