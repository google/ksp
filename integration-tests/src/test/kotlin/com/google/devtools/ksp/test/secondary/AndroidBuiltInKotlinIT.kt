...
class AndroidBuiltInKotlinIT(experimentalPsiResolution: Boolean) {
        ...
        gradleRunner
            ...
            .let { result ->
                ...
                val javaResDir =
                    File(
                        project.root,
                        "workload/build/intermediates/java_res/debug/processDebugJavaRes/out",
                    )
                Assert.assertTrue(File(javaResDir, "TestProcessor.log").exists())
                Assert.assertTrue(
                    File(javaResDir, "META-INF/proguard/builder-AClassBuilder.pro").exists()
                )
                Assert.assertTrue(
                    File(javaResDir, "META-INF/proguard/builder-BClassBuilder.pro").exists()
                )
                assertMergedConfigurationOutput(
                    project,
                    "-keep class com.example.AClassBuilder { *; }",
                )
                assertMergedConfigurationOutput(
                    project,
                    "-keep class com.example.BClassBuilder { *; }",
                )

                val outputs = result.output.lines()
                assert(
                    "w: [ksp] [workload] Mangled name for internalFun: internalFun\$workload" in
                        outputs
                )
                assert(
                    "w: [ksp] [workload] Mangled name for internalFun: internalFun\$workload" in
    ...
    fun testPlaygroundAndroidWithBuiltInKotlinAGP90BelowAlpha14() {
        val gradleRunner =
            GradleRunner.create().withProjectDir(project.root).withGradleVersion("9.0.0")
    ...
    fun `test Kotlin generated sources are picked up by KSP with built-in Kotlin`() {
        ...
        gradleRunner.withArguments(":workload:assembleDebug", "--stacktrace").build().let { result
            ...
            Assert.assertEquals(
                TaskOutcome.SUCCESS,
                result.task(":workload:kspDebugKotlin")?.outcome,
            )

            val logFile =
                File(
                    project.root,
                    "workload/build/generated/ksp/debug/resources/TestProcessor.log",
                )
            Assert.assertTrue(
                "Log file expected to exist but it does not exist: ${logFile.path}",
                logFile.exists(),
            )
            val logContent = logFile.readText()
            Assert.assertTrue(
                logContent.contains("Found GeneratedKotlinClass: com.example.GeneratedKotlinClass")
            )
        }
    }
}
