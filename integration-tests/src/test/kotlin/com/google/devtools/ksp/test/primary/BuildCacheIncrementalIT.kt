...
class BuildCacheIncrementalIT(experimentalPsiResolution: Boolean) {
    ...
    fun testIncrementalBuildCache() {
        val buildCacheDir =
            File(project.root, "build-cache").absolutePath.replace(File.separatorChar, '/')
        ...
        File(project.root, k2).writeText("package p1\n\n@MyAnnotation\nclass K2\n")
        gradleRunner.withArguments("assemble", "--stacktrace").build()

        File(project.root, k2).delete()
        gradleRunner.withArguments("assemble", "--stacktrace").build()

        File(project.root, k1).writeText("package p1\n\nclass K1(val foo: String)\n")
        gradleRunner.withArguments("assemble", "--stacktrace").build()
    }
    ...
    fun testIncrementalBuildCacheCleanOutputsPreserved() {
        ...
        // If it fails with "Unresolved reference 'K2Generated'", it means K2Generated was lost
        // (bug).
        gradleRunner.withArguments("assemble", "--stacktrace").build()
    }
}
