...
class HmppIT(experimentalPsiResolution: Boolean) {
    ...
    val taskToFilesTraditional =
        mapOf(
            ...
            ":workload:kspJvmLinuxX64KotlinMetadata" to
                "w: [ksp] EchoProcessor: CommonMain_JvmLinuxX64",
            ":workload:kspKotlinJvm" to
                "w: [ksp] EchoProcessor: CommonMain_JvmJs_JvmLinuxX64_JvmMain_JvmOnly",
            ":workload:kspKotlinJs" to "w: [ksp] EchoProcessor: CommonMain_JsMain_JvmJs",
            ":workload:kspKotlinLinuxX64" to
                "w: [ksp] EchoProcessor: CommonMain_JvmLinuxX64_LinuxX64Main",
        )
            ...
            gradleRunner
                ...
                .let { result ->
                    val logs =
                        result.output
                            .lines()
                            .filter { it.startsWith("w: [ksp] EchoProcessor: ") }
                            .toSet()
    ...
    val taskToFilesHmpp =
        mapOf(
            ":workload:kspCommonMainKotlinMetadata" to setOf("w: [ksp] EchoProcessor: CommonMain"),
            ":workload:kspJvmJsKotlinMetadata" to
                setOf(
